...
# Test Java wrapper generation for firstBadVersion with user code cleaning
...
def test_java_wrapper():
    ...
    def create_batch_java_wrapper(user_code, test_cases, function_name):
        ...
        # Include VersionControl class for firstBadVersion and clean user code
        if is_first_bad_version:
...
'''
            # Remove "extends VersionControl" from user code if it exists
            user_code = user_code.replace("extends VersionControl", "").replace("  {", " {")
        else:
            version_control_class = ''
...
'''
    ...
    # Test with firstBadVersion function that incorrectly extends VersionControl
    test_code = '''
...
'''
    ...
    print("- Removed extends VersionControl:", "extends VersionControl" not in wrapper.split("BatchSolution")[0])
    print("- Has separate VersionControl class:", wrapper.count("class VersionControl") == 1)
    print()
