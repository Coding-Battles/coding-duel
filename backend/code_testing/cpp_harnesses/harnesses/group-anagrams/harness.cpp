...
static inline string vec2DStrToStr(const vector<vector<string>>& a){
    string s="["; for(size_t i=0;i<a.size();++i){ s+=vecStrToStr(a[i]); if(i+1<a.size()) s+=','; } s+=']'; return s;
}

int main(int argc,char**argv){
    if(argc<3){ cout<<"{\"result\":\"Missing arguments\",\"execution_time\": 0}\n"; return 1; }
    string in=argv[2];
    auto strs=parseStringArray(in,"strs");
    Solution sol;
    auto v=sol.groupAnagrams(strs);
    cout<<"{\"result\": "<<vec2DStrToStr(v)<<", \"execution_time\": 0}\n";
    return 0;
}