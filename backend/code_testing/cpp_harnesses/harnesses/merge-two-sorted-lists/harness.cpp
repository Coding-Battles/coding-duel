...
int main(int argc,char**argv){
    ...
    string in=argv[2];
    auto a=parseIntArray(in,"list1");
    auto b=parseIntArray(in,"list2");
    Solution sol;
    auto node=sol.mergeTwoLists(vecToList(a), vecToList(b));
    auto v=listToVec(node);
    cout<<"{\"result\": "<<vecToStr(v)<<", \"execution_time\": 0}\n";
    return 0;
}