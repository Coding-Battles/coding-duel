...
def setup_java_persistent_server(container, config):
    ...
    server_file_path = os.path.join(
        os.path.dirname(__file__), "PersistentJavaRunner.java"
    )
    ...
    if create_result.exit_code != 0:
        raise Exception(
            f"Failed to copy server code: {create_result.output.decode('utf-8')}"
        )

    # Compile the server
    compile_result = container.exec_run(
        "javac PersistentJavaRunner.java", workdir="/tmp"
    )
    ...
    print(
        "🐛 [DOCKER DEBUG] Setting up persistent Java server with socket communication"
    )
    ...
    if server_result.exit_code != 0:
        raise Exception(
            f"Failed to start Java server background process: {server_result.output.decode('utf-8')}"
        )
    ...
    if pid_check.exit_code == 0:
        ...
        if server_pid.isdigit():
            ...
            # Check server logs for verification (network_mode=none prevents socket tests)
            log_check = container.exec_run("cat /tmp/java_server.log", workdir="/tmp")
            logs = (
                log_check.output.decode("utf-8")
                if log_check.exit_code == 0
                else "No logs available"
            )
            print(f"🐛 [DOCKER DEBUG] Server logs: {logs}")

            # Verify server started by checking logs (more reliable than socket test with network_mode=none)
            if (
                "PersistentJavaRunner started - listening on port 8899" in logs
                or "Socket server listening on port 8899" in logs
            ):
                print(
                    "🐛 [DOCKER DEBUG] Java server confirmed started - logs show server initialization"
                )
                ...
                # Fallback: Try socket test (may fail due to network_mode=none)
                print(
                    "🐛 [DOCKER DEBUG] Server logs don't show startup - trying socket test as fallback"
                )
                test_socket = container.exec_run(
                    'sh -c \'timeout 5 bash -c "</dev/tcp/localhost/8899" 2>/dev/null && echo "Socket OK" || echo "Socket FAIL"\'',
                    workdir="/tmp",
                )
                ...
                if "Socket OK" in socket_status:
                    print("🐛 [DOCKER DEBUG] Java server confirmed via socket test")
                    _prewarm_java_server(container)
                else:
                    raise Exception(
                        f"Java server verification failed. Neither logs nor socket test confirmed startup. Logs: {logs[:500]}"
                    )
        else:
            raise Exception(f"Invalid server PID in file: {server_pid}")
    else:
        raise Exception(
            "Could not read server PID file - server may have failed to start"
        )
...
"""
            ...
            if script_create.exit_code == 0:
                ...
                socket_send = container.exec_run(
                    f"timeout 15 bash /tmp/prewarm_comm_{i}.sh", workdir="/tmp"
                )

                if (
                    socket_send.exit_code == 0 or socket_send.exit_code == 124
                ):  # 124 = timeout
                    ...
                    if output and "success" in output:
                        ...
                        print(
                            f"🔥 [PREWARM] Dummy request {i+1} response: {output[:100]}..."
                        )
                else:
                    print(
                        f"🔥 [PREWARM] Dummy request {i+1} failed with exit code {socket_send.exit_code}"
                    )
