...
def setup_socket_events(
    ...
    async def disconnect(sid):
        ...
        for player_list in [
            waiting_players_easy,
            waiting_players_medium,
            waiting_players_hard,
        ]:
            for player in player_list:
                if player.sid == sid:
                    player_list.remove(player)
                    logger.info(
                        f"Player {player.name} removed from queue due to disconnect"
                    )
                    break
        ...
        for player_id, game_id in player_to_game.items():
            if game_id in game_states:
                ...
                if (
                    ...
                    and game_state.players[player_id].sid == sid
                ):
        ...
        logger.info(
            f"Client {sid} disconnected. Queue size: easy={len(waiting_players_easy)}, medium={len(waiting_players_medium)}, hard={len(waiting_players_hard)}"
        )
    ...
    async def join_queue(sid, data):
        ...
        try:
            ...
            if len(difficulties) > 0:
                ...
                logger.info(
                    f"Pairing players: {player1.name} vs {player2.name} in difficulty {difficulty_identifier}"
                )
                ...
                logger.info(f"difficulty_identifier: {difficulty_identifier}")
                logger.info(f"keys available: {list(data['questions'].keys())}")
                logger.info(
                    f"selected list: {data['questions'][difficulty_identifier]}"
                )
                ...
                question_name = question[
                    "slug"
                ]  # Use slug for both loading files and URLs
                question_slug = question["slug"]  # Use slug for URLs
                question_title = question["title"]  # Human-readable title for display

                logger.info(
                    f"player1: {player1.name} vs player2: {player2.name} with question: {question_name}"
                )
                ...
                game_state = GameState(
                    game_id=game_id,
                    players={
                        player1.id: PlayerInfo(
                            id=player1.id,
                            name=player1.name,
                            sid=player1.sid,
                            anonymous=player1.anonymous,
                        ),
                        player2.id: PlayerInfo(
                            id=player2.id,
                            name=player2.name,
                            sid=player2.sid,
                ...
                logger.info(
                    f"Match found: {player1.name} vs {player2.name} in game {game_id}"
                )
                ...
                status_response = QueueStatusResponse(
                    status="waiting",
                    queue_size=max(
                        len(waiting_players_easy),
                        len(waiting_players_medium),
                        len(waiting_players_hard),
                    ),
                )
    ...
    async def leave_queue(sid, data=None):
        ...
        try:
            ...
            # Remove player from all difficulty queues
            for queue_name, player_list in [
                ("easy", waiting_players_easy),
                ("medium", waiting_players_medium), 
                ("hard", waiting_players_hard)
            ]:
                for player in player_list[:]:  # Copy list to avoid modification during iteration
                    if player.sid == sid:
                        player_list.remove(player)
                        removed = True
                        removed_player_name = player.name
                        logger.info(f"[leave_queue] Removed player {player.name} from {queue_name} queue")
            
            logger.info(
                f"[leave_queue] Queue sizes after removal: easy={len(waiting_players_easy)}, medium={len(waiting_players_medium)}, hard={len(waiting_players_hard)}"
            )
            
            if removed:
                logger.info(f"Player {removed_player_name} with sid {sid} left the queue")
